...
def voting_round(G, round, undecided, alternator):
  nodes_to_votes = nx.get_node_attributes(G, 'vote')
  ...
  for undec in sorted(undecided):
    ...
    if A_voters > B_voters: decision = 'A'
    if A_voters < B_voters: decision = 'B'
...
def with_ads(G, rounds, graph_number):
  ...
  for i in range(10):
    ...
    for round in range(rounds):
      print 'Round', round
      results = voting_round(G, round, undecided, alternator)
      G = results[0]
      alternator = results[1]
    votes_to_nodes = get_votes_to_nodes(G)
...
def no_spending(G1, G2, rounds):
  ...
  for round in range(rounds):
    print 'Round', round
    results = voting_round(G1, round, undecided, alternator)
    G1 = results[0]
    alternator = results[1]
  ...
  for round in range(rounds):
    print 'Round', round
    results = voting_round(G2, round, undecided, alternator)
    G2 = results[0]
    alternator = results[1]
