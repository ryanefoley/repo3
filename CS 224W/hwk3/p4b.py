...
def prop_in_comp(G):
    size = len(G.nodes())
    H=nx.connected_component_subgraphs(G)[0] #subgraph of largest connected component
    size_comp = len(H.nodes())
    return size_comp/float(size)
    ...
    for t in types:
      for g in graphs:
        ...
        while len(g.nodes()) > original_size*(1-(y/100.0)):
          ...
          p = prop_in_comp(g)
          print p
          x_vals += [(1 - len(g.nodes())/float(original_size)) * 100]
          y_vals += [p]

        plt.plot(x_vals, y_vals, 'ro')
        plt.ylabel('Proportion of Nodes in Largest Connected Component')
        plt.xlabel('Percentage of Nodes Removed')
        plt.savefig("prob4b_"+str(i)+".png")
        plt.close()
        i += 1
