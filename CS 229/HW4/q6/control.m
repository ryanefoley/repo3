...
values = rand(NUM_STATES, 1)*.1;

%Number of transitions from one state to another given an action

trans = zeros(NUM_STATES, NUM_STATES, 2);
trans_probs = ones(NUM_STATES, NUM_STATES, 2)/NUM_STATES;
rewards = zeros(NUM_STATES, 1);
total_rewards = zeros(NUM_STATES, 1);
rewards_counter = zeros(NUM_STATES, 1);
...
%this is number of consecutive learning trials < NO_LEARNING_THRESHOLD
num_learning_trials = 0;
while (num_learning_trials < NO_LEARNING_THRESHOLD)
  ...
  % model.
  value_right = trans_probs(state, :, 1) * values;
  value_left = trans_probs(state, :, 2) * values;
  ...
  trans(state, new_state, action) = trans(state, new_state, action) + 1;
  rewards_counter(new_state) = rewards_counter(new_state) + 1;
  total_rewards(new_state) = total_rewards(new_state) + R;
  ...
  if (new_state==NUM_STATES)
    ...
    % Updating transition probabilities and rewards from observations 
    
    for(a=1:2)
      for(s=1:NUM_STATES)
        denom = sum(trans(s,:,a));  
        if denom > 0
          trans_probs(s, :, a) = trans(s, :, a) / denom;
        end
      end
    end
    
    for(s=1:NUM_STATES)
      if rewards_counter(s) > 0
        rewards(s) = total_rewards(s) / rewards_counter(s);
      end
    end
    ...
    % Updating values via value iteration
    iters = 0;
    new_values = zeros(NUM_STATES,1);
    while(true)
      iters = iters + 1;
      iters;
      for (s=1:NUM_STATES)
        future_vals_sum = zeros(2, 1);
        for (a=1:2)
          future_vals_sum(a) = trans_probs(s, :, a)*values;
        end
        new_values(s) = max(future_vals_sum);
      end
      new_values = rewards + GAMMA*new_values;
                
      max_diff = max(abs(values-new_values));
      values = new_values;
    ...
    if iters == 1
      num_learning_trials = num_learning_trials + 1; 
    else
      num_learning_trials = 0;
    end
  end
