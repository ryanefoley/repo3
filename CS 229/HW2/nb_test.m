...
for i = 1:numTestDocs
  ...
  for j = 1:numTokens
    log_prob_spam = log_prob_spam + testMatrix(i,j)*log(spamWordCounts(j)/wordCounts(j));
    log_prob_nonspam = log_prob_nonspam + testMatrix(i,j)*log(nonspamWordCounts(j)/wordCounts(j));
  end
