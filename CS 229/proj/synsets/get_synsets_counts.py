...
def preprocess(reviews):
    ...
    for i in range(len(reviews)):
        ...
        #getting synsets:
        pos_tgs = {'ADJ': 'a', 'ADV': 'r', 'N': 'n', 'NN': 'n', 'NNS': 'n', 'V': 'v', 'VD': 'v', 'VN': 'v', 'VB': 'v','VBD': 'v', 'VBG': 'v', 'VBN': 'v', 'VBP': 'v', 'VBZ': 'v'}
        #tags = nltk.pos_tag(processed)
        #synsets = []
        #for tag in tags:
        #    if tag[1] in pos_tgs:
        #        s =  wn.synsets(tag[0], pos_tgs[tag[1]])
        #        synsets = synsets + s
...
def make_matrix(all_words, reviews, type, fold):
    writer = csv.writer(open('just_filtered_word_counts.dat', 'wb'), delimiter = ' ')
    for i in range(len(reviews)):
